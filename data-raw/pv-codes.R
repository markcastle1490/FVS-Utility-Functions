...
################################################################################
#Valid PV_CODES by region, which then potentially get mapped to valid PV code in
#a variant.
################################################################################

extract_pv = function(file,
                      start_string,
                      end_string,
                      verbose = FALSE)
{
  #Read from pvref file
  values = trimws(readLines(file, warn = FALSE))
  
  #Get indices for starting and ending line to read
  start = match(x = start_string, table = values)
  end = match(x = end_string, table = values)
  
  if(verbose)
  {
    cat("start:", start_string, "\n")
    cat("end:", end_string, "\n")
    cat("start_idx:", start, "\n")
    cat("end_idx:", end, "\n", "\n")
  }
  ...
  #Reset specific values to XXXXXXXXXX so they get dropped later
  output[grepl(pattern = 'DATA', x = output) | 
         grepl(pattern = 'MAP PV/REFERENCE', x = output) |
         grepl(pattern = 'C-', x = output) |
         grepl(pattern = 'KODTYP=0', x = output)] = "XXXXXXXXXX"
  
  #Replace &, /, and ' values
  output = gsub(pattern = "[&/']", replacement = "", x = output)
  output = unlist(strsplit(x = output, split = ","))
  
  #Drop "XXXXXXXXXX", "C", "!"
  output = trimws(output[!output %in% c("XXXXXXXXXX", "C", "!")])
...
#Intialize list for storing pv code information
pv_list = vector(mode = "list", length = length(pvref_files))

#Start loop across pvref_files
for(i in 1:length(pvref_files))
{
  #Get file
  pvref_f = pvref_files[i]
  
  #Get file name
  pvref_ = sub(".*/", "", pvref_f)
  
  #Get variant
  var = toupper(substr(x = pvref_, 
                       start = 1,
                       stop = 2))
  
  #Region
  region_index = nchar(pvref_) - 2
  region = as.integer(substr(x = pvref_, 
                             start = region_index,
                             stop = region_index))
  
  start_1 = 'DATA (HABPVR(I),I=   1,  60)/'
  start_2 = 'DATA (PVCODE(I),I=   1,  60)/'
  start_3 = 'DATA (PVREF(I),I=   1,  60)/'
  
  if(var %in% c('EM', 'IE', 'KT'))
    start_4 = 'C  MAP PV/REFERENCE CODES INTO A FVS HABITAT/ECOCLASS CODE'
  else
    start_4 = 'KODTYP=0'
  
  #Get habpvr
  habpvr = extract_pv(pvref_f,
                      start = start_1,
                      end = start_2,
                      verbose = TRUE)
  
  #Get pvcode
  pvcode = extract_pv(pvref_f,
                      start = start_2,
                      end = start_3,
                      verbose = TRUE)
  
  #Get pvref
  pvref = extract_pv(pvref_f,
                     start = start_3,
                     end = start_4,
                     verbose = TRUE)
  
  #Create dataframe
  pv_df = data.frame(VARIANT = rep(x = var, times = length(habpvr)),
                     REGION = rep(x = region, times = length(habpvr)),
                     HABPVR = habpvr,
                     PVCODE = pvcode, 
                     PVREF = pvref)
  
  #Add datafraeme to list
  pv_list[[i]] = pv_df
  
  #Clean-up
  rm(pv_df, habpvr, pvcode, pvref, pvref_f, pvref_, var, region, region_index)
}

#Bind list
pv_regions = do.call("rbind", pv_list)
